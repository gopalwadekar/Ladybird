...
public class Utilities {
	...
	public static String getIdfromURL(String url, String regx, int idx) {
		String id = "";
		try {
			url = URLDecoder.decode(url);
			String qa[] = url.split("id=");
			for (int i = 0; i < qa.length; i++) {
				if (i == idx) {
					id = URLDecoder.decode(qa[i]);
					break;
				}
			}

			if (!id.isEmpty()) {
				System.out.println("Id Found from URL = " + id);
			} else {
				System.out.println("Id Not Found from URL" + id);
			}

		} catch (Exception e) {
			System.out.println("Excp Found in geeting Id " + e.getMessage());
		}
		...
		try {
			...
			if (parameter.startsWith("auto")) {
				...
				if (!Constants.userID.isEmpty()) {
					d = Constants.userID;
					System.out.println("User Id found in jar : "+Constants.userID);
				}
				else{
					d ="User Id not found in jar";
					System.out.println("User Id not found in jar : "+Constants.userID);
				}
			}
	...
	public static void toSave(boolean toSave, String[] data) {
		...
		if (d) {
			System.out.println("PASSED : Value " + val + " is saved in excel");
		} else {
			System.out.println("FAILED : Value " + val + " is not saved in excel");
		}
	...
	public static String backClick() {
		String url = "";
		try {
			...
			System.out.println("Exception in backClick");
		}
